...
def reindex(df: pd.DataFrame, bipartite: bool = False):
    """
...
def preprocess_data(dataset_name: str, bipartite: bool = True, node_feat_dim: int = 172):
    ...
    #if edge_feats.shape[1] < EDGE_FEAT_DIM:
    #    edge_zero_padding = np.zeros((edge_feats.shape[0], EDGE_FEAT_DIM - edge_feats.shape[1]))
    #    edge_feats = np.concatenate([edge_feats, edge_zero_padding], axis=1)
    #elif edge_feats.shape[1] > EDGE_FEAT_DIM:
    #    edge_feats = edge_feats[:, :EDGE_FEAT_DIM]

    # node features with one additional feature for zero index (since node id starts from 1)
    max_idx = max(new_df.u.max(), new_df.i.max())
    node_feats = np.zeros((max_idx, 24 * 365))

    print('number of nodes ', node_feats.shape[0])
...
parser.add_argument('--dataset_name', type=str,
                    choices=['ieee118'],
                    help='Dataset name', default='ieee118')
parser.add_argument('--node_feat_dim', type=int, default=24*365, help='Number of node raw features')

args = parser.parse_args()

print(f'preprocess dataset {args.dataset_name}...')
# Since ieee118 is not a bipartite dataset, set bipartite=False
...
# No need to check data for ieee118 as it is a custom dataset
